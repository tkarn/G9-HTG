# Version 2023-07-20  Analyses of G9-results and validation cohorts
#
#
#
#' # SETUP
...
suppressWarnings(
  import.data <- read_excel(
    "input/G9_combined_results_all_genes_18JUL2023.xlsx",
    na = c("", "NaN"),
...
fn <- "out/Fig4i_Venn-pCR-immune.pdf"
pdf(file = fn)
...
fn <- "out/Fig4j_Venn-pCR-prolif.pdf"
pdf(file = fn)
ggVennDiagram(genes[c(3, 2, 4, 5)], label_alpha = 0) +
  scale_fill_gradient(low = "white", high = "red") +
  ggtitle(title)
...
fn <- "out/Fig4k_Venn-pCR-stromal.pdf"
pdf(file = fn)
ggVennDiagram(genes[c(3, 2, 4, 5)], label_alpha = 0) +
  scale_fill_gradient(low = "white", high = "red") +
  ggtitle(title)
...
fn <- "out/Fig4p_Venn-DDFS-immune.pdf"
pdf(file = fn)
ggVennDiagram(genes[c(3, 2, 4, 5)], label_alpha = 0) +
  scale_fill_gradient(low = "white", high = "red") +
  ggtitle(title)
...
fn <- "out/Fig4q_Venn-DDFS-prolif.pdf"
pdf(file = fn)
ggVennDiagram(genes[c(3, 2, 4, 5)], label_alpha = 0) +
  scale_fill_gradient(low = "white", high = "red") +
  ggtitle(title)
...
fn <- "out/Fig4r_Venn-DDFS-stromal.pdf"
pdf(file = fn)
ggVennDiagram(genes[c(3, 2, 4, 5)], label_alpha = 0) +
  scale_fill_gradient(low = "white", high = "red") +
  ggtitle(title)
...
fn <- "out/Fig4d_Venn-pCR-DDFS.pdf"
pdf(file = fn)
...
#
fn <- "out/Fig5b_Scatter-pCR-DDFS-Plac.pdf"
...
#
fn <- "out/Fig5c_Scatter-pCR-DDFS-Durva.pdf"
pdf(file = fn, width = 11, height = 8.5)
...
fn <- "out/Fig6h_Scatter-pCR-Durva-Plac-G9.pdf"
...
fn <- "out/Fig6i_Scatter-pCR-Durva-MEDI-NonDurva-Brightn.pdf"
...
fn <- "out/Fig6j_Scatter-pCR-DDFS-MEDI-SCANB.pdf"
...
fn <- "out/Fig6k_Scatter-pCR-DDFS-BRTN-SCANB.pdf"
pdf(file = fn, width = 11, height = 8.5)
...
#
# The signatures will be calculated separately for the two validation
# cohorts (MEDI and BRTN)
...
# The following code used dat and coh
...
sig.pairs <- tribble(
  ~pname, ~x.ax, ~y.ax,
  "Fig6m_MEDI_g1sca_vs_g2sca", "sig.durva.good.g1.immune.scaled", "sig.durva.good.g2.proliferation.scaled",
  "Fig6o_MEDI_g1sca_vs_g4sca", "sig.durva.good.g1.immune.scaled", "sig.durva.poor.g4.stromal.NonImmune.scaled"
)
